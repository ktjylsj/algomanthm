...
var gradient = Kapsule({
    ...
    update: function update(state) {
        ...
        var stopsScale = d3Scale.scaleLinear().domain([0, 100]).range(state.colorScale.domain());

        var colorStops = state.gradient.selectAll('stop').data(d3Array.range(0, 100.01, 20)); // 11 stops is sufficient to cover all noticeable color nuances
...
var dropshadow = Kapsule({
    ...
    init: function init(el, state) {
        state.filter = d3Selection.select(el).append('defs').append('filter').attr('height', '130%');
...
var image = Kapsule({
    ...
    init: function init(el, state) {
        state.img = d3Selection.select(el).append('image');
    },
    update: function update(state) {
...
var zoomOverviewArea = Kapsule({
    ...
    init: function init(el, state, _ref) {
        ...
        // Build dom
        state.svg = d3Selection.select(el).append('svg').attr('width', width).attr('height', height);
