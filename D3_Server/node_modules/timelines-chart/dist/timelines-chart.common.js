...
var ColorLegend = _interopDefault(require('d3-color-legend'));
...
var TimeOverview = Kapsule({
  ...
  init: function init(el, state) {
    ...
    state.brush = d3Brush.brushX().on('end', function () {
      if (!d3Selection.event.sourceEvent) return;
      var selection = d3Selection.event.selection ? d3Selection.event.selection.map(state.scale.invert) : state.scale.domain();
  ...
  init: function init(el, state) {
    ...
    function buildDomStructure() {
      ...
      function addOverviewArea() {
        ...
        state.svg.on('zoomScent', function () {
          var zoomX = d3Selection.event.detail.zoomX;
    ...
    function addZoomSelection() {
      state.graph.on('mousedown', function () {
        if (d3Selection.select(window).on('mousemove.zoomRect') != null) // Selection already active
          return;
        var e = this;
        if (d3Selection.mouse(e)[0] < 0 || d3Selection.mouse(e)[0] > state.graphW || d3Selection.mouse(e)[1] < 0 || d3Selection.mouse(e)[1] > state.graphH) return;
        state.disableHover = true;
        var rect = state.graph.append('rect').attr('class', 'chart-zoom-selection');
        var startCoords = d3Selection.mouse(e);
        d3Selection.select(window).on('mousemove.zoomRect', function () {
          d3Selection.event.stopPropagation();
          var newCoords = [Math.max(0, Math.min(state.graphW, d3Selection.mouse(e)[0])), Math.max(0, Math.min(state.graphH, d3Selection.mouse(e)[1]))];
          ...
          var endCoords = [Math.max(0, Math.min(state.graphW, d3Selection.mouse(e)[0])), Math.max(0, Math.min(state.graphH, d3Selection.mouse(e)[1]))];
    ...
    function setEvents() {
      state.svg.on('zoom', function () {
        var evData = d3Selection.event.detail,
            zoomX = evData.zoomX,
            zoomY = evData.zoomY,
