...
var ContinuousLegend = Kapsule({
    ...
    init: function init(el, state) {
        state.gradient = svgUtils.gradient()(el);

        state.el = d3Selection.select(el);
        ...
        state.labelFitText = svgTextFit.fitToBox()(state.unitLabel.node());

        state.startLabel = state.el.append('text').style('text-anchor', 'start').style('dominant-baseline', 'central');

        state.startLabelFitText = svgTextFit.fitToBox()(state.startLabel.node());

        state.endLabel = state.el.append('text').style('text-anchor', 'end').style('dominant-baseline', 'central');

        state.endLabelFitText = svgTextFit.fitToBox()(state.endLabel.node());
    },
...
var legend = Kapsule({
    ...
    init: function init(el, state) {
        state.legend = d3Selection.select(el).append('g').attr('class', 'legend');
    },
    update: function update(state) {
